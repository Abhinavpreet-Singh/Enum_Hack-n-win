...
function autoDetectFunctionName(code, language) {
  ...
  if (lang === 'java') {
    // Skip 'main' — find the first non-main public/static method
 ...
 *
 * Run complexity analysis on submitted code.
 */
export const analyzeCodeComplexity = async (req, res) => {
  try {
    ...
    // If still no function name and not static-only, warn but continue with static-only
 ...
 *
 * Health check for the complexity analyzer subsystem.
 */
 ...
 *
 * Export ML training data as CSV (admin only).
 */
