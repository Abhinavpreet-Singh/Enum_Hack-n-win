 ...
 *   1. Buffers all stdin lines via fgets (handles empty strings, spaces)
 *   2. Parses each parameter from successive lines
 *   3. Calls the user's function
 *   4. Prints the result in the expected format
 *
 * C conventions for array return types:
 *   - int[]  return → function returns int*  and takes extra int* returnSize
 *   - float[] return → function returns double* and takes extra int* returnSize
 *   - Array params → (type* arr, int arrSize) pair
 */
export const generateCWrapper = ({
  ...
  // Extract #include directives from user code so they are hoisted to
  // the top of the file (C requires them before any declarations)
  ...
  parameterTypes.forEach((type, index) => {
    ...
    // Function params: arrays pass (pointer, size) pair
  ...
  // For array return types, add &_retSize as last argument
  let retSizeDecl = "";
