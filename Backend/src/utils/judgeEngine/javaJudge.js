...
/**
 * Resolve the full paths to `javac` and `java` binaries.
 * Priority:
 *   1. JAVA_HOME environment variable
 *   2. Common Linux JDK locations
...
function resolveJavaBinaries() {
  ...
  // Common install locations on Ubuntu/Debian servers
  const candidates = [
  ...
  // Fall back to bare commands and let the OS resolve via PATH
      ...
      await new Promise((res, rej) => {
        exec(`"${javacCmd}" temp/Main.java`, (err, _stdout, stderr) => {
          if (err) {
            ...
            // Provide a clear hint when the JDK is simply not installed
